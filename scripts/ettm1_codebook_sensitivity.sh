#!/bin/bash
# =====================================================================
# ETTm1 codebook-size sensitivity analysis.
#
# Keep all settings aligned with scripts/ettm1_best.sh, only sweep
# CODEBOOK_SIZE over:
#   128 256 336 512 720 1024
#
# Usage from repo root:
#   bash scripts/ettm1_codebook_sensitivity.sh
#
# Optional override:
#   CODEBOOK_SIZE_LIST="128 256 512" bash scripts/ettm1_codebook_sensitivity.sh
# =====================================================================
...
# Match ettm1_best.sh VQVAE/codebook params except CODEBOOK_SIZE (swept below)
...
# Match ettm1_best.sh short-mode NTP pretrain params
...
# Match ettm1_best.sh short-mode finetune params
...
CODEBOOK_SIZE_LIST="${CODEBOOK_SIZE_LIST:-128 256 336 512 720 1024}"

echo "================================================="
echo "ETTm1 codebook-size sensitivity run"
echo "================================================="
echo "Repo root              : ${REPO_ROOT}"
echo "Codebook size list     : ${CODEBOOK_SIZE_LIST}"
...
for CODEBOOK_SIZE in ${CODEBOOK_SIZE_LIST}; do
    export CODEBOOK_SIZE
    export RUN_HISTORY_PREFIX="${DSET}_cb${CODEBOOK_SIZE}_pred96_freq_m${MAX_CHANNELS_PER_MODEL}_base${BASE_MODEL_ID}"

    echo
    echo "-------------------------------------------------"
    echo "Start sweep item: CODEBOOK_SIZE=${CODEBOOK_SIZE}"
    ...
    rc=$?
    if [ "${rc}" -ne 0 ]; then
        echo "ERROR: run failed for CODEBOOK_SIZE=${CODEBOOK_SIZE} (rc=${rc})"
        exit "${rc}"
    fi
done

echo
echo "All codebook-size sweep runs finished successfully."
